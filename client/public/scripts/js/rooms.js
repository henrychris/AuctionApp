...
var getRuntimeVersion = function() {
  if (Platform.isNode) {
...
function configureFetch(obj) {
  if (typeof fetch === "undefined" || Platform.isNode) {
    ...
    obj._fetchType = (()=>({}))(obj._fetchType, obj._jar);
    return true;
  }
  return false;
}
...
class MessageBuffer {
  ...
  _dispose(error) {
    error !== null && error !== undefined || (error = new Error("Unable to reconnect to server."));
...
class HubConnection {
  ...
  constructor(connection, logger, protocol, reconnectPolicy, serverTimeoutInMilliseconds, keepAliveIntervalInMilliseconds, statefulReconnectBufferSize) {
    ...
    this._statefulReconnectBufferSize = statefulReconnectBufferSize !== null && statefulReconnectBufferSize !== undefined ? statefulReconnectBufferSize : DEFAULT_STATEFUL_RECONNECT_BUFFER_SIZE;
    this._logger = logger;
    this._protocol = protocol;
  ...
  _completeClose(error) {
    if (this._connectionStarted) {
      ...
      this._connectionStarted = false;
      if (this._messageBuffer) {
        this._messageBuffer._dispose(error !== null && error !== undefined ? error : new Error("Connection closed."));
...
class HttpConnection {
  ...
  async _createTransport(url, requestedTransport, negotiateResponse, requestedTransferFormat) {
    ...
    for (const endpoint of transports) {
      const transportOrError = this._resolveTransportOrError(endpoint, requestedTransport, requestedTransferFormat, (negotiate === null || negotiate === undefined ? undefined : negotiate.useStatefulReconnect) === true);
...
class HubConnectionBuilder {
  ...
  withStatefulReconnect(options) {
    ...
    this._statefulReconnectBufferSize = options === null || options === undefined ? undefined : options.bufferSize;
    return this;
  }
  build() {
...
var SetSignalRMessageReceivers = function(connection) {
...
var createRoomRow = function(room) {
...
var addMutationObserver = function() {
  ...
  const config4 = { attributes: true, attributeFilter: ["style"] };
  observer.observe(section, config4);
};
...
var loginRes;
var TOKEN = "";
var storedData = JSON.parse(localStorage.getItem("loginRes"));
loginRes = storedData;
if (loginRes) {

...
(() => {
  ...
  // (disabled):node_modules/tough-cookie/lib/cookie.js
  ...
  // (disabled):node_modules/fetch-cookie/cjs/index-wrapper.js
  var require_index_wrapper = __commonJS({
    "(disabled):node_modules/fetch-cookie/cjs/index-wrapper.js"() {
    }
  });

  // (disabled):node_modules/abort-controller/browser.js
  ...
  // (disabled):node_modules/eventsource/lib/eventsource.js
  ...
  function configureFetch(obj) {
    if (typeof fetch === "undefined" || Platform.isNode) {
      obj._jar = new (require_cookie()).CookieJar();
      if (typeof fetch === "undefined") {
        obj._fetchType = require_browser();
      } else {
        obj._fetchType = fetch;
      }
      obj._fetchType = require_index_wrapper()(obj._fetchType, obj._jar);
      return true;
    }
    return false;
  }
  function configureAbortController(obj) {
    if (typeof AbortController === "undefined") {
      obj._abortControllerType = require_browser2();
      return true;
    }
    return false;
  }
  ...
  function SetSignalRMessageReceivers(connection2) {
    connection2.on("ReceiveMessage", function(msg) {
      console.log("hit");
      const messages = document.getElementById("messages");
      const user = `<span style="font-weight: bold">${msg.userName}: </span>`;
      messages.innerHTML += `<p>${user}<span>${msg.content}</span></p>`;
    });
    connection2.on("AuctionStarted", function(msg) {
      console.log("hit");
      const messages = document.getElementById("messages");
    ...
    connection2.on(
      "AuctionEnded",
      function(winnerFirstName, amountInNaira) {
        const messages = document.getElementById("messages");
        const user = `<span style="font-weight: bold">Admin: </span>`;
    ...
    connection2.on("KickUser", function(user) {
      const messages = document.getElementById("messages");
  ...
  async function getAuctionData(roomId, TOKEN2) {
    var res = await GetDataWithToken(BASE_URL + `/rooms/${roomId}/data`, TOKEN2);
    return res.data;
  }
  async function GetUserDetails(userId, TOKEN2) {
    const res = await GetDataWithToken(`${BASE_URL}/user/${userId}`, TOKEN2);
    return res.data;
  }
